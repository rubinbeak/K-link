...
export function BrandBudgetCalculator() {
  ...
  const price = useMemo(() => {
    ...
    const subtotal = subUnder + subOver;
    ...
    <section className="relative pt-2 sm:pt-4">
      <div className="relative text-center">
        <p className="flex items-center justify-center gap-1.5 text-xs font-semibold text-primary">
          <Calculator className="size-3.5 opacity-80 shrink-0" aria-hidden />
          즉시 예산 계산
        </p>
        <h3 className="mt-3 text-balance font-heading text-2xl font-semibold tracking-tight">
          캠페인 예상 비용을 바로 확인해보세요
        </h3>
      ...
      <div className="relative mt-6 grid gap-4 lg:grid-cols-[1fr_0.95fr]">
        <div className="space-y-4">
          <div className="rounded-xl border border-zinc-200/60 bg-zinc-50/40 p-4">
            ...
            <p className="mt-2 text-xs text-muted-foreground">
              5천 미만 구간은 1인 기준 {money.format(UNIT_LTE_5K)}원 단가가 적용됩니다.
            </p>
          </div>
        ...
        <div className="rounded-xl bg-primary px-4 py-4 text-primary-foreground shadow-sm">
            ...
            {price.headcount > 0 && (
              <>
                <p className="flex items-center justify-between border-t border-white/20 pt-2 text-primary-foreground/90">
                  <span>공급가액 합계</span>
                  <span className="shrink-0 font-medium tabular-nums">{money.format(price.subtotal)}원</span>
                </p>
                <p className="flex items-center justify-between text-primary-foreground/90">
                  <span>부가세 (10%)</span>
                  <span className="shrink-0 font-medium tabular-nums">{money.format(price.vat)}원</span>
                </p>
                <p className="flex items-center justify-between text-primary-foreground/90">
                  <span>모집 합계</span>
          ...
          <div className="mt-4 rounded-lg bg-white/12 p-4">
            <p className="text-xs text-primary-foreground/80">예상 총액 (VAT 포함)</p>
            <p className="mt-1 text-3xl font-semibold tracking-tight text-white tabular-nums">
              {money.format(price.totalWithVat)}원
            </p>
          </div>
